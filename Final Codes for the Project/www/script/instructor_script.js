////////////////////////// Script needed for the instuctor interface

$(document).ready(function(){
    $('#result-btn').addClass('disabled');
    
    $('#ques-modal').modal({backdrop: 'static', keyboard: false, show: false}); //////////// Modal dispaly, prevent from clicking outside
    ...
    $('#result-btn').click(function(){ //////////// View class results
    ...
    $('#individual-result-btn').click(function(){ //////////// View individual results
        $('#individual-result-graph').empty();
        errc=0; ////////////// Error flag for course
        erru=0; ////////////// Error flag for USN
        $('#course-error, #usn-error').text('');
        if($('#view-result-for').val()=="individual"){
            ...
            if($('#usn').val()==""){
                ...
                $.post('/api/validate_usn/', usnResult, function(data, success){
                    if(data=="invalid"){
                        ...
                        if(errc==0 && erru==0){
                            $('#individual-result-graph').append('<iframe src="/api/get_results/for_instructor/2/graph/index.php?course='+$('#select-course-result-1').val()+'&usn='+$('#usn').val()+'" height="370" width="600" style="border:0;"></iframe>');
                        }
                    }
                });
            }
            ...
            if($('#select-course-result-1').val()==null){
                ...
                $('#individual-result-graph').append('<iframe src="/api/get_results/for_instructor/2/table/index.php?course='+allResult.course+'&class='+allResult.class+'" height="370" width="600" style="border:0;"></iframe>');
...
function uploadQuestions(){
    minQues=30; //////////// Minimun number of questions
    ...
    if($("#upload-form").val()!=""){
        ...
        if(filetype!="pdf" && filetype!="txt" && filetype!="doc" && filetype!="docx"){ //////////// Check for file types
            $("#error-file-text").text("Please select only PDF, Text, Doc and Docx files");
            errt=1;
        }
    ...
    if($("#upload-form2").val()!=""){
        ...
        if(filetype2!="pdf" && filetype2!="txt" && filetype2!="doc" && filetype2!="docx"){ //////////// Check for file types
...
function validateQuestions(){
    ...
    if(i<minQues){
        $("#ques-error").text("Must contain minimum 30 questions. ");
        errq+=1;
    }
    ...
    if($('#date-time').val()==''){
        ...
        if(date_time.localeCompare(current_time)<=0){
            ...
            $('#post-quiz-btn').text("Posting quiz...");
