<?php
////////////////// API to get the list of quizzes
session_start();
...
if($in_login=="yes"){ ////////////////// For instructor
    ...
    $result = mysqli_query($conn,"SELECT t.date, count(*) as event_count FROM test t, list_of_students l WHERE l.USN='$st_id' and l.class_id=t.class_id GROUP BY date");
    while($row = mysqli_fetch_array($result)){
        if($first_time!=0)
            $outstr.=',';
