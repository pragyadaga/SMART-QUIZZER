...
class SummaryTool(object):
    ...
    def get_summary(self, content, sentences_dic):
        ...
        # for p in paragraphs:
        sentence = self.get_best_sentence(content, sentences_dic).strip()
        if sentence:
...
def main(file_name, content):
    ...
    # Print the summary
    f = open(file_name, "a")
    f.write(summary.encode('cp850', errors='replace').decode())
    f.write(".\n")
    f.close()
    #print (summary.encode('cp850', errors='replace').decode())
